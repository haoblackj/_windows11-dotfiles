...
winget install Google.JapaneseIME --source winget && winget install Microsoft.PowerShell --source winget && winget install Biscuit.Biscuit --source winget && winget install Google.Chrome --source winget && winget install Mozilla.Firefox --source winget && winget install Microsoft.Powertoys --source winget --scope machine && winget install 8bitSolutionsLLC.Bitwarden --source winget && winget install Bandisoft.Honeyview --source winget && winget install voidtools.Everything --source winget && winget install SumatraPDF.SumatraPDF --source winget && winget install TeraTermProject.teraterm5 --source winget
...
if ($homeMachine -ieq "Yes" -or $homeMachine -ieq "y") {
    ...
    winget install AdGuard.AdGuard --source winget && winget install CubeSoft.CubePDF --source winget
